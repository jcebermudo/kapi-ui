...
export function insertOverlay() {
  ...
  const swapCopyIcon = (svg: string, done?: () => void) => {
  ...
  copyBtn.addEventListener('click', async () => {
    ...
    swapCopyIcon(CHECK_SVG, () =>
      setTimeout(() => swapCopyIcon(COPY_SVG, () => copyBtn.classList.remove('kapi-busy')), 1000),
    )
  })
  ...
  let logoSwapTimeout: ReturnType<typeof setTimeout> | null = null
  let logoSwapInnerTimeout: ReturnType<typeof setTimeout> | null = null

  const clearPendingLogoSwap = () => {
    if (logoSwapTimeout !== null) clearTimeout(logoSwapTimeout)
    if (logoSwapInnerTimeout !== null) clearTimeout(logoSwapInnerTimeout)
    logoSwapTimeout = null
    logoSwapInnerTimeout = null
  }

  const swapLogo = (svg: string) => {
