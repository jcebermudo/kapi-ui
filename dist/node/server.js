...
let session = null; // codex only — claude tracks its own session id below
let portPromise = null;
let serverStarted = false;
const processBySocket = new WeakMap(); // codex only
...
// -----------------------------------------------------------------------------
async function processCodexComments(prompt, socket, cwd) {
            ...
            socket.on('message', (data) => {
                try {
                    ...
                    if (message.type === 'comments:submit') {
                        if (agent === 'claude')
                            processClaudeComments(message.prompt, socket);
                        else
                            void processCodexComments(message.prompt, socket, cwd);
                    }
