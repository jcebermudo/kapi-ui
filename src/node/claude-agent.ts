...
import type { AgentRuntime, AgentClient } from './types.js'
...
const pendingPrompts: Array<{ prompt: string; client: AgentClient | null }> = []
...
function ensureClaudeProc(): ChildProcess | null {
  ...
  claude.stdout!.on('data', (chunk) => {
    ...
    while ((newlineIndex = buffer.indexOf('\n')) !== -1) {
      ...
      if (status) activeClient?.send('kapi:processing', { status })
    }
  })
...
function processQueue() {
  ...
  if (!claude || !claude.stdin) {
    ...
    for (const { client } of stranded) client?.send('kapi:done')
    return
  }
...
export const claudeAgent: AgentRuntime = {
  start(startCwd) {
    cwd = startCwd
  ...
  stop(client) {
    ...
    for (let i = pendingPrompts.length - 1; i >= 0; i--) {
      if (pendingPrompts[i]!.client === client) pendingPrompts.splice(i, 1)
    }
