...
function adjustPanelPosition(marker: HTMLElement, panel: HTMLElement) {
...
function repositionAll() {
  ...
  wrappers.forEach((wrapper, i) => {
    ...
    const marker = wrapper.querySelector<HTMLElement>('.kapi-comment-marker')
    if (!marker) return
    const composer = wrapper.querySelector<HTMLElement>('.kapi-comment-composer')
    if (composer) {
      adjustPanelPosition(marker, composer)
      return
    }
    const tooltip = wrapper.querySelector<HTMLElement>('.kapi-comment-tooltip')
    if (tooltip && wrapper.classList.contains('kapi-hovering')) adjustPanelPosition(marker, tooltip)
  })
}
