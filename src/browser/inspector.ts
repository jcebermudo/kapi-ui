...
let onSelectionChange: ((els: Element[]) => void) | null = null
...
let outlineTargets: Element[] = []
let selectionOutlineEls: HTMLDivElement[] = []
...
function paintMarquee(x1: number, y1: number, x2: number, y2: number) {
...
export function previewElements(els: Element[]) {
  outlineTargets = els
  paintOutlines()
}
...
function elementsInBox(x1: number, y1: number, x2: number, y2: number): Element[] {
  ...
  for (let y = top; y <= bottom; y += BOX_SELECT_STEP) {
    for (let x = left; x <= right; x += BOX_SELECT_STEP) {
      ...
      if (stack[0]?.closest(IGNORE_SELECTOR)) continue
...
function selectElementsInBox(x1: number, y1: number, x2: number, y2: number, additive: boolean) {
  if (!additive) selected.clear()
  for (const el of elementsInBox(x1, y1, x2, y2)) selected.add(el)
  notifySelectionChange()
}

function handleBlockerPointerDown(e: PointerEvent) {
...
function handleBlockerPointerMove(e: PointerEvent) {
...
function handleBlockerPointerUp(e: PointerEvent) {
  if (!boxDragStart) return
  if (boxDragging) {
...
export function isBoxSelectClick(): boolean {
...
function handlePointerMove(e: PointerEvent) {
  ...
  // handler clears it.
  if (locked || boxDragging || boxDragStart) return
